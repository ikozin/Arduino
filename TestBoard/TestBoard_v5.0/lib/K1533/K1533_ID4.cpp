...
const static TDevicePin pin_map_1533_id4_1[] =
{
  // Входы      1   2   &   &                 Выходы      0   1   2   3
  { .Input = { 13,  3, 14, 15, 0, 0, 0, 0 }, .Output = {  9, 10, 11, 12, 0, 0, 0, 0 }},
};

const static TDevicePin pin_map_1533_id4_2[] =
{
  // Входы      1   2   &   &                 Выходы      0   1   2   3
  { .Input = { 13,  3,  1,  2, 0, 0, 0, 0 }, .Output = { 7 ,  6,  5,  4, 0, 0, 0, 0 }},
};

const static TDeviceVal values_1533_id4_1[] =
{  
...
const static TDeviceVal values_1533_id4_2[] =
{  
...
void K1533ID4::info(void) {
  Serial.println(F("KP1533ИД4"));
  Serial.println(F("2 Дешифратора"));
  Serial.println(F("DIP16"));
  Serial.println(F("\t         16 - +5V        "));
  Serial.println(F("\t          8 - GND        "));
  Serial.println(F("\t      --------------     "));
  Serial.println(F("\t 14 -o &  | DC |  0 o- 9 "));
  Serial.println(F("\t 15 -o S0 |    |  1 o-10 "));
  Serial.println(F("\t     |----|    |  2 o-11 "));
  Serial.println(F("\t 13 -| 1  |    |  3 o-12 "));
  Serial.println(F("\t  3 -| 2  |    |----|    "));
  Serial.println(F("\t     |----|    |  0 o- 7 "));
  Serial.println(F("\t  1 -| &  |    |  1 o- 6 "));
  Serial.println(F("\t  2 -o S1 |    |  2 o- 5 "));
