...
bool StorageRadio::load(void) {
    File f = LittleFS.open(FS_RADIOLIST_FILE);
    LOGN("StorageRadio::load");
    if (f) {
        String text;
        int data;
        text.reserve(64);
...
bool StorageRadio::add(RadioItem* item) {
    if (_size == _capacity) {
...
bool StorageRadio::remove(int index) {
