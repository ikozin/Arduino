...
ControllerRadio::ControllerRadio(const char* name, Preferences* prefs) : Controller(name) {
    _prefs = prefs;
    _currentVolume = 0;
    _currentIndex  = 0;
    _isMute = false;
...
void ControllerRadio::OnHandle() {
    _radio.Init();    
    setRadio(_prefs->getInt("station", 35));
...
void ControllerRadio::setRadio(uint16_t index) {
  if (index >= listSize) index = 0;
  if (_currentIndex == index) return;
  _currentIndex = index;
  RadioItem_t rec = radioList[_currentIndex];
